...
app.get('/screenshot', async (req, res) => {
  ...
  try {
    const response = await axios.get(url, { timeout: 20000, httpAgent, httpsAgent, headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36', 'Accept-Language': 'fr-FR,fr;q=0.9,en-US;q=0.8,en;q=0.7', 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8', 'Referer': 'https://www.facebook.com/' }, responseType: 'text', maxRedirects: 5 });
    const html = response.data;
    const ogImgMatch = html.match(/<meta[^>]+property=["']og:image["'][^>]+content=["']([^"']+)["']/i);
    if (ogImgMatch) return await serveImage(res, ogImgMatch[1]);
    const imgMatches = [...html.matchAll(/<img[^>]+src=["']([^"']+)["']/gi)];
    const fbcdnImages = imgMatches.map(m => m[1]).filter(s => (s.includes('fbcdn.net') || s.includes('cdninstagram.com')) && !s.includes('emoji') && !s.includes('icon')).map(s => s.replace(/\/s\d+x\d+\//, '/s1080x1080/').replace(/[\/?&]oh=[^&]+/g, '').replace(/[\/?&]oe=[^&]+/g, '').replace(/[\/?&]s=\d+/g, ''));
  ...
  } catch (err) { res.status(502).json({ status: 'error', message: 'Failed: ' + err.message }); }
});
...
app.get('/api/video', async (req, res) => {
  ...
  try {
    const r = await axios.get(url, { timeout: 20000, httpAgent, httpsAgent, headers: { 'User-Agent': 'Mozilla/5.0', 'Accept-Language': 'fr-FR,fr;q=0.9', 'Referer': 'https://www.facebook.com/' }, responseType: 'text' });
    const html = r.data;
...
app.get('/media', async (req, res) => {
  ...
  try {
    const r = await axios.get(url, { timeout: 20000, httpAgent, httpsAgent, headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36', 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8', 'Referer': 'https://www.facebook.com/' }, responseType: 'text', maxRedirects: 5 });
    const html = r.data;
    const imgs = [];
    const og = html.match(/<meta[^>]+property=["']og:image["'][^>]+content=["']([^"']+)["']/i);
    if (og?.[1]) imgs.push(og[1]);
    const all = [...html.matchAll(/<img[^>]+src=["']([^"']+)["']/gi)].map(m => m[1]).filter(s => (s.includes('fbcdn.net')||s.includes('cdninstagram.com'))&&!s.includes('emoji')&&!s.includes('icon')).map(s => s.replace(/\/s\d+x\d+\//,'/s1080x1080/').replace(/[\/?&]oh=[^&]+/g,'').replace(/[\/?&]oe=[^&]+/g,'').replace(/[\/?&]s=\d+/g,''));
    for (const u of all) { if (imgs.length>=5) break; if (!imgs.includes(u)) imgs.push(u); }
    if (imgs.length===0) { const vp = html.match(/poster=["']([^"']+)["']/i); if (vp?.[1]) imgs.push(vp[1]); }
    const patterns = [/"playable_url"\s*:\s*"([^"]+)"/, /"browser_native_hd_url"\s*:\s*"([^"]+)"/, /"browser_native_sd_url"\s*:\s*"([^"]+)"/, /"hd_src"\s*:\s*"([^"]+)"/, /"sd_src"\s*:\s*"([^"]+)"/, /<meta[^>]+property=["']og:video["'][^>]+content=["']([^"']+)["']/i, /"video_url"\s*:\s*"([^"]+)"/];
    let vid = null;
    for (const p of patterns) { const m = html.match(p); if (m) { vid = m[1].replace(/\\u003d/g,'=').replace(/\\u0026/g,'&').replace(/\\\//g,'/'); break; } }
    res.json({ data: { video: vid, images: imgs } });
...
app.get('/view', async (req, res) => {
  ...
  // Use JSON.stringify for proper JS escaping
  const snapUrlJs = JSON.stringify(snapUrl);
...
.media-box{background:#0f1420;border:1px solid #1e2235;border-radius:16px;padding:20px;max-width:800px;margin:0 auto 20px}
.media-box img{max-width:100%;border-radius:8px;display:block;margin:0 auto}
.media-box video{max-width:100%;border-radius:8px}
...
<div id="app">
  ...
  <div class="tabs">
    ...
    <div class="tab" :class="{on:tab==='media'}" @click="tab='media';fetchMedia()">🎬 Media (video/images)</div>
  </div>
  
  <div class="media-box" v-if="tab==='screenshot'">
    <a :href="screenshotUrl" target="_blank" style="display:block;margin-bottom:10px;color:#818cf8;font-size:13px">📸 Open in new tab</a>
    <img :src="screenshotUrl" alt="Screenshot" onerror="this.outerHTML='<p style=color:#fca5a5>No screenshot available</p>'">
  </div>
  
  <div class="media-box" v-if="tab==='media'">
    <div v-if="loading"><div class="spinner"></div></div>
    <div v-if="videoUrl">
      <video :src="videoUrl" controls autoplay muted style="width:100%"></video>
      <p style="color:#818cf8;margin-top:10px">✅ Video found</p>
    </div>
    <div v-if="images.length">
      <img v-for="(img,i) in images" :key="i" :src="proxyUrl+'?url='+encodeURIComponent(img)" style="max-width:100%;margin:10px 0;border-radius:8px" @error="$event.target.style.display='none'">
    </div>
    <p v-if="!loading&&!videoUrl&&!images.length" style="color:#6b7280">No media found for this ad</p>
  </div>
...
createApp({setup(){
  ...
  const images=ref([]);
  const snapUrl=${snapUrlJs};
  const proxyUrl='${BASE}/proxy-image';
  const screenshotUrl='${BASE}/screenshot?url='+encodeURIComponent(snapUrl)+'&waitFor=3000';
  async function fetchMedia(){loading.value=true;try{const j=await(await fetch('${BASE}/media?url='+encodeURIComponent(snapUrl))).json();videoUrl.value=j?.data?.video||j?.video||null;images.value=j?.data?.images||j?.images||[]}catch(e){}loading.value=false}
  return{tab,loading,videoUrl,images,snapUrl,proxyUrl,screenshotUrl,fetchMedia}
}}).mount('#app');
</script>
</body></html>`);
});
...
async function serveImage(res, imgUrl) {
  try {
    const r = await axios.get(imgUrl, { timeout: 15000, httpAgent, httpsAgent, responseType: 'arraybuffer', headers: { 'User-Agent': 'Mozilla/5.0', 'Accept': 'image/webp,image/apng,image/*,*/*;q=0.8', 'Referer': 'https://www.facebook.com/' }, maxRedirects: 5 });
    res.set({ 'Content-Type': r.headers['content-type'] || 'image/jpeg', 'Cache-Control': 'public, max-age=86400', 'Content-Length': r.data.length });
    res.send(r.data);
  } catch (err) {
...
app.listen(parseInt(process.env.PORT || '3000', 10), '0.0.0.0', () => {
  ...
  console.log('  /screenshot  /media  /api/video  /proxy-image  /view  /health');
});